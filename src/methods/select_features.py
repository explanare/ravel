...
def select_features_with_classifier(autoencoder, inputs, labels, coeff=None):
  if coeff is None:
    coeff = [0.1, 10, 100, 1000]
  coeff_to_select_features = {}
  device = autoencoder.encoder[0].weight.device
  for c in coeff:
    with torch.no_grad():
      X_transformed = autoencoder.encode(torch.tensor(
          inputs, device=device)).cpu().numpy()
