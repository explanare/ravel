...
class RAVELMetadata:
  ...
  def sample_entities(self, split, k):
    ...
    return random.sample(self.split_to_entities[split],
                         k=min(k, len(self.split_to_entities[split])))


def gen_context_test_split(metadata,
                           extract_label_fn,
                           filter_example_fn,
                           first_n=256):
  ...
  for prompt, (attr, split) in metadata.attr_prompt_to_split.items():
    if split == 'train' or attr == 'Other':
      continue
    base_task_inputs = [((prompt, entity),
                         metadata.prompt_to_output[prompt % entity])
    ...
    for subsplit, filter_fn in subsplits_filter.items():
      ...
      source_task_inputs = []
      for source_prompt, (
          source_attr, source_split) in metadata.attr_prompt_to_split.items():
        if not (source_split == split and filter_fn(source_attr)):
          continue
        source_entities = []
        if source_attr == 'Other' and metadata.entity_prompt_to_split[
      ...
      source_task_inputs_weights = np.array(
          source_task_inputs_weights) / np.sum(source_task_inputs_weights)
      ...
      print(
          attr, prompt, split, len(base_task_inputs),
          len(
              set([
                  e['entity'] for e in
                  eval_split_to_raw_example[f'{prompt}-{subsplit}-{split}']
...
def gen_entity_test_split(metadata,
                          extract_label_fn,
                          filter_example_fn,
                          first_n=256):
  eval_split_to_raw_example = {}
    ...
    for split in ('test', 'val'):
      ...
      for subsplit, filter_fn in subsplits_filter.items():
        source_task_inputs = [
            ...
            for source_prompt, (
                source_attr,
                source_split) in metadata.attr_prompt_to_split.items()
        ...
        eval_split_to_raw_example[f'{prompt}-{subsplit}-{split}'] = [{
            ...
            'entity': a,
            'source_entity': s_a
        } for (p, a), v in base_task_inputs for (
            s_p, s_a), source_v in random.choices(
                source_task_inputs,
                weights=source_task_inputs_weights,
                k=max(1, round(first_n /
                               len(base_task_inputs)))) if filter_example_fn(
                                   v, metadata.prompt_to_output[p % s_a])]
  eval_split_to_raw_example = {
      k: v for k, v in eval_split_to_raw_example.items() if len(v) > 0
  }
  return eval_split_to_raw_example
...
def gen_train_split(metadata, extract_label_fn, filter_example_fn, first_n=256):
  ...
  for attr, prompt_to_split in metadata.attr_to_prompt.items():
    ...
    base_task_inputs = [
        ...
        for prompt in random.sample(base_prompt_candiates,
                                    k=min(10, len(base_prompt_candiates)))
    ]
    source_task_inputs = [
        ((source_prompt, entity),
         metadata.prompt_to_output[source_prompt % entity])
    ...
    wiki_source_task_inputs = [
        ((source_prompt, entity),
         metadata.prompt_to_output[source_prompt % entity]) for source_prompt,
        split_and_arg in metadata.entity_prompt_to_split.items()
        if split_and_arg['split'] == target_split and
        (split_and_arg['entity'] is None or
         split_and_arg['entity'] in metadata.get_entities(target_split))
        for entity in ([split_and_arg['entity']] if split_and_arg['entity'] else
                       metadata.sample_entities(target_split, k=10))
    ]
    source_task_inputs = source_task_inputs + wiki_source_task_inputs
    if len(base_task_inputs) < 5 or len(source_task_inputs) < 5:
      continue
    print(attr, target_split, len(base_task_inputs), len(source_task_inputs),
    ...
    for (p, a), v in base_task_inputs:
      ...
      split_to_raw_example[f'{attr}-{target_split}'].extend([{
          ...
          source_input_candiates,
          k=min(len(source_input_candiates),
